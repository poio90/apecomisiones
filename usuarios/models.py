...
class User(AbstractUser):
    ...
    regex_num_af = RegexValidator(
        regex=r'(?:(\d{5})([/]\d{1})?)',
        message=_("El número de afiliado debe ser ingresado en formato: 99999/9 o 99999. Hasta 7 caracteres son permitidos")
    )
    ...
    regex_dni = RegexValidator(
        regex=r'^\d{8}(?:[-\s]\d{4})?$',
        message=_("El DNI debe ser ingresado en formato: 09999999 o 99999999. Hasta 8 caracteres permitidos")
    )
    ...
    dni = models.CharField(
        ...
        null=True,
        help_text=_(
            _('El DNI debe ser ingresado en formato: 09999999 o 99999999. Hasta 8 caracteres permitidos')
        ),
    )

    regex_tel = RegexValidator(
        regex=r'^(?:\(?\d{2,3}\)?[- .]?\d{4,5}[- .]?\(?\d{4}\)?)',
        message=_("El número de telefono debe ser ingresado sin 0 y 15, no se permiten letras. Hasta 10 digitos permitidos")
    )

    num_tel = models.CharField(
        _('Número de Telefono'),
